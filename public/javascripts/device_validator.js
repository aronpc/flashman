(function() {
  let deviceValidator = (function () {
    let validateRegex = function(value, length, regex) {
      let valid = true;
      let err = [];
    ...
    Validator.prototype.validateUser = function(user) {
      ...
      let ret = validateRegex(user, 64, /^[a-zA-Z0-9\-\_\#\s]+$/);
    ...
    Validator.prototype.validatePassword = function(pass) {
      ...
      let ret = validateRegex(pass, 64, /^[a-zA-Z0-9\-\_\#\!\@\$\%\&\*\=\+\?]+$/);
      ret.err = ret.err.map(ind => messages[ind]);
      return ret;
    };
    ...
    Validator.prototype.validateSSID = function(ssid) {
      ...
      let ret = validateRegex(ssid, 32, /^[a-zA-Z0-9\-\_\#\s]+$/);
      ret.err = ret.err.map(ind => messages[ind]);
      return ret;
    };
